...
export default function QuestionFlow({ category, onComplete, onBack, lang, onToggleLang }: QuestionFlowProps) {
  ...
  return (
    <div
      ...
      style={{ opacity: exiting ? 0 : 1, transition: exiting ? 'opacity 0.5s ease' : 'none' }}
    >
      <div
      ...
      <div className="relative z-10 max-w-2xl mx-auto w-full pt-28 pb-16 flex flex-col flex-1">
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-body font-medium tracking-wider uppercase" style={{ color: '#C4973B' }}>
              {category.label}
            </span>
            <span className="text-xs font-body text-gray-600">{T.step2of2}</span>
          </div>
          <ProgressBar current={currentQ + 1} total={category.questions.length} label={T.questionLabel} />
        </div>
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            <div
              ...
              style={{ background: 'rgba(196,151,59,0.15)', border: '1px solid rgba(196,151,59,0.3)', color: '#D4A94B' }}
            >
              {currentQ + 1}
            </div>
          ...
          <h2
            ...
            style={{ fontSize: 'clamp(1.3rem, 3vw, 1.8rem)', color: '#F5F5F0' }}
          >
            {question.text}
          </h2>
            ...
            {question.answers.map((answer, idx) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? 'rgba(196,151,59,0.15)' : 'rgba(19,19,30,0.7)',
                    border: isSelected ? '1px solid rgba(196,151,59,0.5)' : '1px solid rgba(196,151,59,0.1)',
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      ...
                      style={{
                        border: isSelected ? '2px solid #C4973B' : '2px solid rgba(196,151,59,0.3)',
                        background: isSelected ? 'rgba(196,151,59,0.2)' : 'transparent',
                      }}
                    >
                      {isSelected && <div className="w-2 h-2 rounded-full" style={{ background: '#C4973B' }} />}
                    </div>
                    <span
                  ...
                  {isSelected && (
                    <div className="absolute right-5 top-1/2 -translate-y-1/2" style={{ color: '#C4973B' }}>
        ...
        {selected === null && (
          <p className="text-center text-xs font-body text-gray-700 mt-8">{T.selectHint}</p>
        )}
        {selected !== null && !isLast && (
          <p className="text-center text-xs font-body mt-8" style={{ color: '#C4973B' }}>{T.loadingNext}</p>
        )}
        {selected !== null && isLast && (
          <p className="text-center text-xs font-body mt-8" style={{ color: '#C4973B' }}>{T.generating}</p>
        )}
      </div>
    </div>
  );
